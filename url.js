...
app.post("/api/create-short-url",function(request,response){
	...
	let sql = `INSERT INTO shorten links(longurl,shorturlid) VALUES('${request.body.longurl}','${uniqueID}')`;
...
app.get("/api/get-all-short-urls",function(request,response){
	let sql = `SELECT * FROM shorten links`;
	con.query(sql,function(error,result){
		if(error){
			response.status(500).json({
				status:"notok",
...
app.get("/:shorturlid",function(request,response){
	...
	let sql = `SELECT * FROM shorten links WHERE shorturlid='${shorturlid}' LIMIT 1`;
	con.query(sql,function(error,result){
		if(error){
			response.status(500).json({
				status:"notok",
			...
			sql = `UPDATE shorten links SET count=${result[0].count+1} WHERE id='${result[0].id}' LIMIT 1`;
